                   ...
                   <form action="<?php htmlspecialchars($_SERVER['PHP_SELF']) ?>"  method="post" enctype="multipart/form-data" class="">
                        <div class="w-full flex flex-col items-center justify-center ">
                            <div class="w-24 h-24 object-cover rounded-full overflow-hidden border-2 border-gray-600">
                                <img src="../img/<?php echo isset($row['photo']) ? $row['photo'] : 'user-profile.jpg' ?>" id="imgDisplay" class="h-full w-full object-cover hover:scale-[1.1] duration-300" >
                            </div>
                            <div class="relative ">
                        ...
                        <div class="grid grid-cols-2 gap-2 mt-4">
                            <div>
                                ...
                                <input type="text" value="<?php echo isset($row['username']) ? $row['username']: '' ?>"  name="username" id="username" class="bg-transparent border-2 border-gray-700 rounded-md px-2 py-2 w-full outline-none focus:border-blue-500 duration-300 text-gray-400" >
                            </div>
                            <div>
                                <label for="email" class="text-blue-500">Email*</label><br>
                                <input type="text" value="<?php echo isset($row['email']) ? $row['email']: '' ?>"  name="email" id="email" class="bg-transparent border-2 border-gray-700 rounded-md px-2 py-2 w-full outline-none focus:border-blue-500 duration-300 text-gray-400" >
                            </div>
                                    ...
                                    <?php
                                        ...
                                        if ($result->num_rows > 0) {
                                            while ($row = $result->fetch_assoc()) {
                                                ...
                                                <option 
                                                    value='" . $row['id'] . "' 
                                                    selected='". ($row['id'] == $row['role_name'] ? "selected" : ''). "'
                                                >
                                                ". ($row['role_name']) ."
                                                </option>";
                                            }
